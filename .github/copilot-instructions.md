...
# GitHub Copilot Terminal Command Auto Approve

For convenience we allow read-only commands to be auto-approved.  There are specific rules in settings.json that define which commands can be auto-approved.

Before generating a command, check the rules in .github/settings.json to see if the command can be auto-approved.  Wherever possible, generate commands that can be auto-approved to streamline the user experience.

# Utilities and Ancillary Tasks

## Generating Random Strings

Use the script `gen_random.sh` to generate random alphanumeric strings. This is useful for creating unique names for resources, temporary directories, or other identifiers.
...
Temp directories should be named "temp_xxxxxxxx" where "xxxxxxxx" is a random alphanumeric string generated by the `gen_random.sh` script.

# Terminal Command Guidelines

## Overview

When generating terminal commands for the user, follow these guidelines:
- No compound commands (i.e., avoid using `&&`, `;`, or piping `|`)

## No Compound Commands

When generating terminal commands for the user, avoid using compound commands such as `&&`, `;`, or piping `|`. Each command should be standalone to ensure clarity and ease of execution.

For example, instead of generating:

```bash
cd /workspaces/juno31_azure-iac-agentic-toolkit/terraform_final && terraform validate
```

Generate separate commands that are individually executed:
```bash
cd /workspaces/juno31_azure-iac-agentic-toolkit/terraform_final
